...
st.markdown('<div class="main-title">NeuroTrade v14</div>', unsafe_allow_html=True)
...
with dashboard_tab:
    ...
    st.markdown('<div class="soft-box"><strong>Short-term scan</strong><br><span class="small-note">Today % and 2-day run % help spot cooling pullbacks after short runs. Trend strength tells you the broader path, while entry quality tells you whether today is actually buyable.</span></div>', unsafe_allow_html=True)
    display_cols = [
    ...
    for col, (_, row) in zip(cols, top.iterrows()):
        badge_class = "pill-good" if row["entry_quality"] in ["Strong Buy", "Weak Buy"] else "pill-wait" if row["entry_quality"] == "Hold / Watch" else "pill-risk"
        col.markdown(
...
with settings_tab:
    ...
    st.markdown(
        f'<div class="soft-box"><strong>NeuroTrade v14</strong><br>'
