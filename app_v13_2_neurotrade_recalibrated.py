...
st.set_page_config(page_title="NeuroTrade V13.1", layout="wide")
...
ASSET_CONFIG = {
    "BTC-USD": {
        "preferred_buy_price": 79000.0,
        "max_chase_pct": 0.035,
        "hard_overextended_pct": 0.075,
        "rsi_buy_min": 40,
        "rsi_buy_max": 55,
        "rsi_hot": 62,
        "support_weight": 0.30,
    ...
    "SOL-USD": {
        "preferred_buy_price": 79.70,
        "max_chase_pct": 0.032,
        "hard_overextended_pct": 0.070,
        "rsi_buy_min": 38,
        "rsi_buy_max": 54,
        "rsi_hot": 61,
        "support_weight": 0.28,
...
st.markdown(
    ...
    div[data-testid="stMetricLabel"] {{ color: {ACCENT} !important; font-weight: 700; }}
    div[data-testid="stMetricValue"] {{ color: #aaff9c !important; font-size: 1.35rem !important; font-weight: 900; text-shadow: 0 0 10px rgba(170,255,156,0.22); }}
    .header-box {{
...
def infer_preferred_buy(symbol: str, history: pd.DataFrame, current_price: float, support: float) -> float:
    ...
    manual_preferred = safe_float(cfg.get("preferred_buy_price"), 0.0)
    if history.empty:
        return round(manual_preferred if manual_preferred > 0 else current_price * 0.97, 2)
    ...
    # NeuroTrade preferred buy should behave like an active pullback zone,
    # not a deep-support wishlist price. Keep manual anchors dominant.
    if manual_preferred > 0:
        preferred = (
            manual_preferred * 0.72
            + ema21 * 0.18
            + low20 * 0.07
            + support * 0.03
        )

        # If the trend is still hot, do not drag the entry down too much.
        if ema21 >= manual_preferred * 0.99:
            preferred = manual_preferred * 0.82 + ema21 * 0.18

        # If price is already trading below the anchor, allow only a modest adjustment.
        if current_price < manual_preferred * 0.985:
            preferred = manual_preferred * 0.88 + min(current_price, ema21) * 0.12

        # ATR can nudge the zone slightly, but not reshape it.
        preferred *= 1 - min(max(atr_pct * 0.08, 0.0), 0.006)

        floor = manual_preferred * 0.982
        ceiling = manual_preferred * 1.008
        preferred = clamp(preferred, floor, ceiling)
        return round(preferred, 2)

    adaptive = (low20 * 0.45) + (ema21 * 0.35) + (support * 0.20)
    adaptive *= 1 - min(max(atr_pct * 0.10, 0.0), 0.008)
    floor = max(current_price * 0.90, support * 0.995)
    ceiling = min(current_price * 0.998, ema21 * 1.003)
    return round(clamp(adaptive, floor, ceiling), 2)
...
def entry_quality_label(score: float) -> str:
    if score >= 72:
...
def score_row(history: pd.DataFrame, symbol: str) -> dict:
    ...
    entry_score = 50.0
    max_chase_pct = safe_float(cfg.get("max_chase_pct"), 0.040) * 100
    hard_overextended_pct = safe_float(cfg.get("hard_overextended_pct"), 0.085) * 100
    rsi_buy_min = safe_float(cfg.get("rsi_buy_min"), 40)
    rsi_buy_max = safe_float(cfg.get("rsi_buy_max"), 56)
    rsi_hot = safe_float(cfg.get("rsi_hot"), 63)

    if dist_buy_pct <= -1.0:
        entry_score += 18
    elif dist_buy_pct <= 0.8:
        entry_score += 12
    elif dist_buy_pct <= max_chase_pct:
        entry_score += 2
    elif dist_buy_pct <= hard_overextended_pct:
        entry_score -= 18
    else:
        entry_score -= 32
    ...
    if rsi_buy_min <= rsi <= rsi_buy_max:
        entry_score += 14
    elif rsi_buy_max < rsi < rsi_hot:
        entry_score -= 8
    elif rsi >= rsi_hot:
        entry_score -= 22
    elif rsi < max(32, rsi_buy_min - 6):
        entry_score -= 10

    if support_gap_pct <= 2.5:
        entry_score += 10
    elif support_gap_pct > 5.0:
        entry_score -= 10

    if resistance_room_pct < 6.0:
        entry_score -= 10
    if resistance_room_pct < 4.0:
        entry_score -= 8
    ...
    if dist_buy_pct > max_chase_pct and rsi >= max(rsi_buy_max + 2, 58):
        entry_score -= 16
    if dist_buy_pct > max_chase_pct and resistance_room_pct < 8.0:
        entry_score -= 12
    if price > ema21 * 1.02 and today_pct > 0:
        entry_score -= 8
    ...
    return {
        ...
        "entry_quality": entry_quality_label(entry_score),
        "buy_score": buy_score,
...
st.markdown('<div class="sub-title">NeuroTrade thinks like market neurons: selective trend context, disciplined entry logic, and a fast workstation.</div>', unsafe_allow_html=True)
...
st.caption(f"Using cached market scan from {st.session_state.get('scan_timestamp', '-')}. The dashboard stays fast while the workstation loads only the selected ticker.")

m1, m2, m3, m4 = st.columns(4)
with m1:
    st.metric("Tracked symbols", len(scan_df))
with m2:
    st.metric("Good entry zones", int(scan_df["entry_quality"].eq("Good Entry Zone").sum()))
with m3:
    st.metric("Avg buy score", f"{scan_df['buy_score'].mean():.1f}")
with m4:
    st.metric("Constructive trends", int(scan_df["trend_strength"].isin(["Strong Uptrend", "Constructive"]).sum()))
...
with right_summary:
    st.markdown(f'<div class="soft-box"><strong>{stock_text}</strong><br><span class="small-note">Fast dashboard, deeper selected-ticker view.</span></div>', unsafe_allow_html=True)

dashboard_tab, workstation_tab, saved_tab, settings_tab = st.tabs(["Dashboard", "Workstation", "Saved Tools", "Settings"])

with dashboard_tab:
    st.markdown('<div class="section-header">Scan results</div>', unsafe_allow_html=True)
    display_cols = [
        "best_buy_today_rank", "symbol", "asset_class", "price", "trend_strength", "entry_quality", "buy_score",
        "today_change_pct", "two_day_run_pct", "preferred_buy_price", "distance_from_buy_pct",
        "setup_quality", "pullback_quality", "risk_adjusted_entry", "extension_score", "trend_entry_gap",
    ]
    ...
    for col in [
        "buy_score", "today_change_pct", "two_day_run_pct", "distance_from_buy_pct", "setup_quality",
    ...
    for col, (_, row) in zip(cols, top.iterrows()):
        badge_class = "pill-good" if row["entry_quality"] == "Good Entry Zone" else "pill-wait" if row["entry_quality"] == "Wait for Pullback" else "pill-risk"
        col.markdown(
            ...
            f'<span class="small-note">Buy score {row["buy_score"]:.1f} | Today {row["today_change_pct"]:+.2f}% | 2-day run {row["two_day_run_pct"]:+.2f}%</span></div>',
            unsafe_allow_html=True,
        )
...
with settings_tab:
    ...
    st.markdown(
        f'<div class="soft-box"><strong>NeuroTrade v12 speed architecture</strong><br>'
        f'<span class="small-note">Dashboard scans only the watchlist and caches the result. Workstation loads deep analysis only for the selected ticker. News is manual. Sunset background stays, while headers and cards use sky-blue accents. Project folder: {BASE_DIR}</span></div>',
        unsafe_allow_html=True,
    )
