...
ACCENT = "#7DEBFF"
ACCENT_SOFT = "rgba(125, 235, 255, 0.16)"
ACCENT_BORDER = "rgba(125, 235, 255, 0.42)"
GOLD = "#FFE46B"
DEFAULT_WATCHLIST = [
...
st.markdown(
    ...
    div[data-testid="stMetric"] {{
        background: linear-gradient(180deg, rgba(32, 51, 64, 0.90) 0%, rgba(25, 31, 46, 0.95) 100%);
    ...
    .header-box {{
        ...
        border: 1px solid {ACCENT_BORDER}; background: linear-gradient(180deg, rgba(24,48,62,0.88) 0%, rgba(20,26,38,0.95) 100%);
    ...
    .gold-box {{
        ...
        background: linear-gradient(180deg, rgba(84, 54, 12, 0.88) 0%, rgba(40, 29, 18, 0.95) 100%);
        border: 1px solid rgba(255, 228, 107, 0.38); color: #fff4b1;
        box-shadow: 0 12px 28px rgba(0,0,0,0.22);
    }}
    .soft-box {{
        border-radius: 18px; padding: 1rem; margin-bottom: 0.75rem;
        background: linear-gradient(180deg, rgba(30, 44, 58, 0.86) 0%, rgba(24, 28, 40, 0.95) 100%);
        border: 1px solid {ACCENT_BORDER}; color: #effcff;
    }}
    .stat-card {{
        border-radius: 18px; padding: 0.95rem; margin-bottom: 0.75rem;
        background: linear-gradient(180deg, rgba(31, 50, 66, 0.90) 0%, rgba(20, 27, 40, 0.96) 100%);
        border: 1px solid {ACCENT_BORDER}; color: #effcff;
...
def infer_preferred_buy(history: pd.DataFrame, current_price: float, support: float) -> float:
    if history.empty:
    ...
    preferred = (support * 0.45) + (ema21 * 0.25) + (low20 * 0.30)
    preferred *= 1 - min(max(atr_pct * 0.18, 0.0), 0.02)
    preferred = clamp(preferred, current_price * 0.85, current_price * 0.997)
...
def score_row(history: pd.DataFrame, symbol: str) -> dict:
    ...
    preferred_buy = infer_preferred_buy(history, price, support)
    ...
    if dist_buy_pct <= 0:
        entry_score += 18
    elif dist_buy_pct <= 2.5:
        entry_score += 12
    elif dist_buy_pct <= 5.0:
        entry_score += 2
    elif dist_buy_pct <= 8.0:
        entry_score -= 12
    else:
        entry_score -= 24
    ...
    if support_gap_pct <= 3.0:
        entry_score += 8
    elif support_gap_pct > 8.0:
        entry_score -= 12

    if resistance_room_pct < 4.0:
        entry_score -= 16
    elif resistance_room_pct < 7.0:
        entry_score -= 6

    if two_day_run >= 4 and today_pct < 0:
        entry_score += 10
    elif two_day_run >= 4 and today_pct > 0.5:
        entry_score -= 10

    entry_score = clamp(entry_score, 0, 100)

    setup_quality = round((trend_score * 0.45) + (entry_score * 0.55), 1)
    pullback_quality = round(clamp(50 + min(two_day_run, 8) * 3 - max(today_pct, 0) * 2 + (8 - max(dist_buy_pct, 0)) * 2, 0, 100), 1)
    ...
    extension_score = round(clamp(max(dist_buy_pct, 0) * 6 + max(rsi - 60, 0) * 1.8 + max(4 - resistance_room_pct, 0) * 6, 0, 100), 1)
...
def scan_watchlist(symbols: tuple[str, ...], daily_period: str) -> pd.DataFrame:
    ...
    if df.empty:
        return df
    df = df.sort_values(["buy_score", "entry_quality_score", "trend_strength_score"], ascending=[False, False, False]).reset_index(drop=True)
...
st.markdown('<div class="sub-title">Fast dashboard, deep workstation. Sunset theme, sky-blue accents, and short-term entry math built to stay selective.</div>', unsafe_allow_html=True)
...
with workstation_tab:
    ...
    if chart_history.empty:
        ...
        chart_history = chart_history.tail(400).copy()
