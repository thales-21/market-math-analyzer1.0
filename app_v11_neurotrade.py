...
DATA_FEED_OPTIONS = ["Yahoo Finance"]
ASSET_CONFIG = {
...
.main-title { color: #fffdf6; font-size: 2.15rem; font-weight: 900; margin-bottom: 0.15rem; text-shadow: 0 0 18px rgba(111,232,255,0.34); }
.sub-title { color: #ffe6cf; margin-bottom: 1rem; }
.overview-box, .accent-card, .accent-card-soft, .alert-card, .stat-card, .summary-card {
    border-radius: 18px;
    padding: 0.95rem 1rem;
    box-shadow: 0 12px 28px rgba(0, 0, 0, 0.22);
}
.overview-box {
    background: linear-gradient(180deg, rgba(24, 33, 40, 0.92) 0%, rgba(22, 26, 36, 0.98) 100%);
    border: 1px solid rgba(111, 232, 255, 0.24);
    margin-bottom: 0.8rem;
}
.accent-card {
    background: linear-gradient(180deg, rgba(61, 44, 10, 0.96) 0%, rgba(44, 30, 8, 0.98) 100%);
    border-left: 6px solid #6FE8FF;
    color: #eafcff;
    box-shadow: 0 0 18px rgba(111, 232, 255, 0.18);
}
.accent-card-soft {
    background: linear-gradient(180deg, rgba(27, 36, 46, 0.95) 0%, rgba(20, 28, 35, 0.98) 100%);
    border-left: 6px solid #7cf2ff;
    color: #effcff;
}
.crypto-summary-card {
    background: linear-gradient(180deg, rgba(20, 46, 59, 0.96) 0%, rgba(16, 34, 44, 0.99) 100%);
    border-left: 6px solid #6FE8FF;
    color: #eafcff;
    box-shadow: 0 0 22px rgba(111, 232, 255, 0.22);
    border-radius: 18px; padding: 0.95rem 1rem;
}
.stock-summary-card {
    background: linear-gradient(180deg, rgba(25, 42, 45, 0.96) 0%, rgba(19, 31, 34, 0.99) 100%);
    border-left: 6px solid #6FE8FF;
    color: #ebfff9;
    border-radius: 18px; padding: 0.95rem 1rem;
}
...
.small-note { color: #fff1e0; }

div[data-testid="stMetric"] {
    background: linear-gradient(180deg, rgba(17, 35, 44, 0.95) 0%, rgba(14, 28, 36, 0.98) 100%);
    border-radius: 18px;
    padding: 0.85rem;
    border: 1px solid rgba(111, 232, 255, 0.34);
    box-shadow: 0 0 18px rgba(111, 232, 255, 0.14);
}
div[data-testid="stMetricLabel"] * {
    color: #7FEFFF !important;
    font-weight: 800 !important;
    text-shadow: 0 0 10px rgba(111, 232, 255, 0.40);
}
div[data-testid="stMetricValue"] * {
    color: #ecfdff !important;
    font-weight: 900 !important;
}
div[data-testid="stMetricDelta"] * { color: #c9f8ff !important; }

.stat-card {
    background: linear-gradient(180deg, rgba(21, 32, 37, 0.95) 0%, rgba(17, 24, 31, 0.98) 100%);
    border: 1px solid rgba(127, 248, 214, 0.18);
}
.stat-label { color: #8fffe0; font-size: 0.90rem; font-weight: 800; margin-bottom: 0.25rem; }
.stat-value { color: #fff8d7; font-size: 1.38rem; font-weight: 900; line-height: 1.2; }
.stat-sub { color: #dceff3; font-size: 0.88rem; margin-top: 0.25rem; }
...
.section-cyan { background: rgba(40, 69, 79, 0.72); color: #a8f7ff; }
.section-gold { background: rgba(88, 67, 18, 0.76); color: #fff0a2; }
.section-magenta { background: rgba(84, 38, 73, 0.76); color: #ffd2ef; }
.section-green { background: rgba(28, 64, 33, 0.76); color: #baff7f; }

.news-banner {
    background: linear-gradient(180deg, rgba(59, 47, 12, 0.94) 0%, rgba(44, 34, 9, 0.98) 100%);
    border-left: 6px solid #6FE8FF;
    color: #fff4b9;
    border-radius: 16px;
...
def render_section_header(title: str, variant: str = "section-cyan") -> None:
    st.markdown(f'<div class="section-header {variant}">{title}</div>', unsafe_allow_html=True)

def clean_dt_index(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return df
    out = df.copy()
...
def compute_preferred_buy_zone(preferred_buy: float) -> tuple[float, float]:
    if preferred_buy <= 0:
        return 0.0, 0.0
    band = max(preferred_buy * 0.006, 0.01)
    return preferred_buy - band, preferred_buy + band

def nice_price_step(price_range: float) -> float:
    if price_range <= 0:
        return 1.0
    raw = price_range / 10
    for step in [0.01,0.02,0.05,0.10,0.20,0.50,1,2,5,10,20,50,100,200,500,1000]:
        if raw <= step:
            return float(step)
    return float(max(1000, round(raw)))
...
def get_symbol_history(symbol: str, period: str, feed: str = "Yahoo Finance") -> pd.DataFrame:
    # NeuroTrade v11 keeps scans on Yahoo Finance by default for speed and consistency.
...
def notify_breaking_news_from_scan(result: pd.DataFrame, include_news: bool, fresh_scan: bool) -> None:
    # Disabled in NeuroTrade v11 to keep scans fast and avoid noisy watchlist polling.
    return
...
def build_advanced_chart(history: pd.DataFrame, symbol: str, preferred_buy: float, support: float, resistance: float, overlays: Dict[str, bool], show_rsi: bool, show_macd: bool) -> go.Figure:
    history = clean_dt_index(history)
    has_volume = "Volume" in history.columns
    rows = 1 + int(has_volume) + int(show_rsi) + int(show_macd)
    heights = [0.60] + ([0.14] if has_volume else []) + ([0.13] if show_rsi else []) + ([0.13] if show_macd else [])
    ...
    fig.add_trace(
        go.Candlestick(
            ...
            increasing_line_color="#92ff5c", increasing_fillcolor="rgba(146,255,92,0.65)",
            decreasing_line_color="#ff7589", decreasing_fillcolor="rgba(255,117,137,0.55)"
        ),
        row=1, col=1
    )
    for col, enabled in overlays.items():
        if enabled and col in history.columns:
            color = {"SMA20":"#ffe45e","SMA50":"#7cf2ff","EMA9":"#96ff96","EMA21":"#ffb86b","EMA50":"#d6a8ff"}.get(col, "#ffffff")
            fig.add_trace(go.Scatter(x=history.index, y=history[col], mode="lines", name=col, line=dict(width=1.5, color=color)), row=1, col=1)
    pb_low, pb_high = compute_preferred_buy_zone(preferred_buy)
    if preferred_buy > 0:
        fig.add_hrect(y0=pb_low, y1=pb_high, fillcolor="rgba(255,230,94,0.16)", line_width=0, row=1, col=1)
        fig.add_hline(y=preferred_buy, line_dash="dot", line_color="#ffe45e", annotation_text="Preferred Buy", row=1, col=1)
    if support > 0:
        fig.add_hrect(y0=support*0.996, y1=support*1.004, fillcolor="rgba(99,255,190,0.10)", line_width=0, row=1, col=1)
        fig.add_hline(y=support, line_dash="dash", line_color="#73ffd8", annotation_text="Support", row=1, col=1)
    if resistance > 0:
        fig.add_hrect(y0=resistance*0.996, y1=resistance*1.004, fillcolor="rgba(255,112,146,0.08)", line_width=0, row=1, col=1)
        fig.add_hline(y=resistance, line_dash="dash", line_color="#ff8ca1", annotation_text="Resistance", row=1, col=1)
    current_row = 2
    if has_volume:
        fig.add_trace(go.Bar(x=history.index, y=history["Volume"], name="Volume", marker_color="rgba(124,242,255,0.45)"), row=current_row, col=1)
        current_row += 1
    if show_rsi and "RSI14" in history.columns:
        fig.add_trace(go.Scatter(x=history.index, y=history["RSI14"], mode="lines", name="RSI14", line=dict(color="#7cf2ff", width=1.6)), row=current_row, col=1)
        fig.add_hline(y=70, line_dash="dash", line_color="#ffb86b", row=current_row, col=1)
        fig.add_hline(y=30, line_dash="dash", line_color="#96ff96", row=current_row, col=1)
        current_row += 1
    if show_macd and "MACD" in history.columns and "MACD_SIGNAL" in history.columns:
        fig.add_trace(go.Scatter(x=history.index, y=history["MACD"], mode="lines", name="MACD", line=dict(color="#ffe45e", width=1.5)), row=current_row, col=1)
        fig.add_trace(go.Scatter(x=history.index, y=history["MACD_SIGNAL"], mode="lines", name="MACD Signal", line=dict(color="#ff8ca1", width=1.5)), row=current_row, col=1)
    y_min = float(history["Low"].tail(250).min()) if not history.empty else 0.0
    y_max = float(history["High"].tail(250).max()) if not history.empty else 1.0
    dtick = nice_price_step(max(y_max - y_min, 0.01))
    fig.update_layout(
        height=940,
        title=f"{symbol} live workstation chart",
        xaxis_rangeslider_visible=False,
        margin=dict(l=20, r=20, t=50, b=20),
        dragmode="pan",
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(18,23,31,0.92)",
        font=dict(color="#f7f9fb"),
        legend=dict(orientation="h", yanchor="bottom", y=1.01, x=0),
    )
    fig.update_xaxes(showgrid=True, gridcolor="rgba(255,255,255,0.05)", zeroline=False, fixedrange=False)
    fig.update_yaxes(showgrid=True, gridcolor="rgba(255,255,255,0.05)", zeroline=False, fixedrange=False, tickformat=",.2f", dtick=dtick)
    return fig
...
def overview_html(symbol: str, selected_row: pd.Series, profile: dict, feed: str) -> str:
    return f'<div class="overview-box"><strong>{profile.get("short_name", symbol)}</strong> ({symbol})<br><span class="small-note">Price: ${safe_float(selected_row.get("price"),0):,.2f} | Decision: {selected_row.get("decision","-")} | Entry score: {safe_float(selected_row.get("entry_score"),0):.1f} | Strength score: {safe_float(selected_row.get("strength_score"),0):.1f}<br>Market cap: {format_value(profile.get("market_cap"))} | Volume: {format_value(profile.get("volume"))} | 52W High: {format_value(profile.get("fifty_two_week_high"))} | 52W Low: {format_value(profile.get("fifty_two_week_low"))}</span></div>'


def render_stat_card(label: str, value: str, sub: str = "", variant: str = "") -> str:
    klass = "stat-card" + (f" {variant}" if variant else "")
    return f'<div class="{klass}"><div class="stat-label">{label}</div><div class="stat-value">{value}</div><div class="stat-sub">{sub}</div></div>'


def derive_trend_strength_label(mtf_score: float) -> str:
    if mtf_score >= 72:
...
def derive_entry_quality_label(distance_from_buy_pct: float, rsi: float, resistance_gap_pct: float) -> str:
    if distance_from_buy_pct <= 1.0 and 40 <= rsi <= 58 and resistance_gap_pct >= 8:
        return "Good Entry"
    if distance_from_buy_pct <= 3.0 and rsi <= 62 and resistance_gap_pct >= 5:
        return "Fair Entry"
    if distance_from_buy_pct <= 6.0 and rsi < 66:
        return "Early / Watch"
    return "Overextended"


def compute_formula_scores(row: pd.Series) -> dict:
    price = safe_float(row.get("price"), 0.0)
    preferred = safe_float(row.get("preferred_buy_price"), price)
    support = safe_float(row.get("support"), price * 0.97 if price else 0.0)
    resistance = safe_float(row.get("resistance"), price * 1.05 if price else 0.0)
    rsi = safe_float(row.get("rsi_14"), 50.0)
    entry_score = safe_float(row.get("entry_score"), 50.0)
    mtf_score = safe_float(row.get("mtf_score"), 50.0)
    atr_pct = safe_float(row.get("atr_pct"), 0.03)
    buy_day = safe_float(row.get("buy_day_score"), 50.0)

    distance_pct = ((price - preferred) / preferred * 100) if preferred else 0.0
    support_gap = ((price - support) / support * 100) if support else 0.0
    resistance_gap = ((resistance - price) / price * 100) if price and resistance else 8.0

    extension_score = round(clamp(100 - max(distance_pct, 0) * 7 - max(rsi - 58, 0) * 2.2 - (6 if resistance_gap < 6 else 0), 0, 100), 1)

    risk = max(abs(price - support), max(price * max(atr_pct, 0.01), 0.01))
    reward = max(resistance - price, 0.0)
    rr = reward / risk if risk > 0 else 1.0
    risk_adjusted_entry = round(clamp(35 + rr * 18 + (6 if 40 <= rsi <= 58 else -6) + (8 if distance_pct <= 2 else -8) + (mtf_score - 50) * 0.35, 0, 100), 1)

    pullback_quality = round(clamp(0.45 * entry_score + 0.25 * buy_day + 0.15 * extension_score + 0.15 * (100 - max(support_gap, 0) * 6), 0, 100), 1)

    trend_entry_divergence = round(max(mtf_score - entry_score, 0), 1)
    setup_quality = round(clamp(0.30 * mtf_score + 0.30 * entry_score + 0.20 * buy_day + 0.20 * risk_adjusted_entry, 0, 100), 1)

    return {
        "extension_score": extension_score,
        "risk_adjusted_entry": risk_adjusted_entry,
        "pullback_quality": pullback_quality,
        "trend_entry_divergence": trend_entry_divergence,
        "setup_quality": setup_quality,
    }


def add_visual_labels(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return df
    out = df.copy()
    out["trend_strength"] = out.get("mtf_score", 50).apply(lambda x: derive_trend_strength_label(safe_float(x, 50)))
    def _entry(row):
        price = safe_float(row.get("price"), 0.0)
        resistance = safe_float(row.get("resistance"), 0.0)
        gap = ((resistance - price) / price * 100) if price and resistance else 99.0
        return derive_entry_quality_label(safe_float(row.get("distance_from_buy_pct"), 0.0), safe_float(row.get("rsi_14"), 50.0), gap)
    out["entry_quality_label"] = out.apply(_entry, axis=1)
    formula_df = out.apply(lambda row: pd.Series(compute_formula_scores(row)), axis=1)
    out = pd.concat([out, formula_df], axis=1)
    return out
...
def resample_ohlcv(df: pd.DataFrame, rule: str) -> pd.DataFrame:
    if df.empty:
        return df
    data = clean_dt_index(df)
    agg = {"Open":"first","High":"max","Low":"min","Close":"last"}
    if "Volume" in data.columns:
        agg["Volume"] = "sum"
    resampled = data.resample(rule).agg(agg).dropna(subset=["Open","High","Low","Close"])
    return add_technicals(resampled)

@st.cache_data(ttl=30, show_spinner=False)
def fetch_yfinance_chart_bars(symbol: str, period: str, interval: str) -> pd.DataFrame:
    try:
        df = yf.download(symbol, period=period, interval=interval, auto_adjust=True, progress=False, prepost=False)
        if df is None or df.empty:
            return pd.DataFrame()
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = [c[0] for c in df.columns]
...
def get_chart_history(symbol: str, chart_window: str, bar_interval: str, feed: str) -> pd.DataFrame:
    interval_map = {
        "5m": "5m",
        "15m": "15m",
        "30m": "30m",
    ...
    if feed in {"Polygon", "Finnhub", "Alpaca", "Twelve Data"} and bar_interval == "1d":
        history = get_symbol_history(symbol, chart_window, feed)

    if history.empty:
        history = fetch_yfinance_chart_bars(symbol, chart_window, interval_map.get(bar_interval, "60m"))

    if history.empty:
        history = get_symbol_history(symbol, chart_window, "Yahoo Finance")

    history = clean_dt_index(history)
    if history.empty:
        return history
    if bar_interval == "4h":
        return resample_ohlcv(history, "4h")
    return add_technicals(history)
...
def compute_short_term_buy_context(history: pd.DataFrame, selected_row: pd.Series) -> dict:
    ...
    if buy_day_score >= 74:
        buy_day_label = "Good Entry Zone"
    elif buy_day_score >= 58:
        buy_day_label = "Wait for Pullback"
    else:
        buy_day_label = "No Setup"
    ...
    summary = (
        ...
        f"Trend score is {mtf_score:.1f} and entry score is {entry_score:.1f}. Buy Score is secondary and mainly helps rank dip setups."
    )
    ...
    return {
        ...
        "trend_strength": derive_trend_strength_label(mtf_score),
        "entry_quality": derive_entry_quality_label(safe_float(selected_row.get("distance_from_buy_pct"), 0.0), rsi, ((safe_float(selected_row.get("resistance"), current_price) - current_price)/current_price*100) if current_price else 99.0),
        "buy_day_hint": "Secondary rank only" if buy_day_label != "Good Entry Zone" else "Trend and entry align",
        "summary": summary,
    }
...
st.markdown('<div class="main-title">NeuroTrade</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-title">Cleaner, faster swing-trading workstation focused on calibrated entries, clean math, and organized decision support.</div>', unsafe_allow_html=True)

DEFAULT_PREFS = {"history_period":"1y","chart_period":"5d","show_sma20":True,"show_sma50":True,"show_ema9":True,"show_ema21":True,"show_ema50":False,"show_rsi":True,"show_macd":True,"include_news":False,"feed_provider":"Yahoo Finance","auto_refresh":False,"refresh_seconds":30}
...
with st.sidebar:
    ...
    feed_provider = st.selectbox("Data source", DATA_FEED_OPTIONS, index=DATA_FEED_OPTIONS.index(str(prefs.get("feed_provider","Yahoo Finance"))))
    ...
    if st.button("Refresh chart view", use_container_width=True):
        st.rerun()
    st.caption("NeuroTrade v11 is tuned for speed and consistency, so scans use Yahoo Finance by default.")
...
if needs_scan:
    with st.spinner("Scanning calibrated swing setups..."):
...
result = add_visual_labels(result)
if result.empty:
...
notify_breaking_news_from_scan(filtered, include_news, needs_scan)
...
with card1: st.markdown(f'<div class="crypto-summary-card"><strong>{crypto_title}</strong><br><span class="small-note">{crypto_detail}</span></div>', unsafe_allow_html=True)
with card2: st.markdown(f'<div class="stock-summary-card"><strong>{stock_title}</strong><br><span class="small-note">{stock_detail}</span></div>', unsafe_allow_html=True)

st.caption('Scan status: ' + ('fresh scan completed' if needs_scan else 'reusing cached market scan') + ' | Feed: Yahoo Finance | News scanner: selected ticker only')

tab1, tab2, tab3, tab4 = st.tabs(["Dashboard","Workstation","Saved Tools","Settings"])
with tab1:
    render_section_header("Top setups", "section-green")
    top = filtered.sort_values(["best_buy_today_rank","entry_score"], ascending=[False,False]).head(12)
    cols = [c for c in ["symbol","asset_class","price","decision","trend_strength","entry_quality_label","best_buy_today_rank","buy_day_score","buy_day_label","setup_quality","pullback_quality","extension_score","risk_adjusted_entry","entry_score","strength_score","preferred_buy_price","distance_from_buy_pct","today_change_pct","two_day_run_pct","mtf_label","news_sentiment_label"] if c in top.columns]
    st.dataframe(top[cols], use_container_width=True, hide_index=True)
    render_section_header("Share-ready summary", "section-cyan")
    st.text_area("Copy summary", value=share_text, height=220)
    render_section_header("Alert center", "section-magenta")
    if alerts_df.empty:
...
with tab2:
    render_section_header("Ticker workstation", "section-cyan")
    if auto_refresh:
    ...
    if not symbols:
        ...
        chart_interval_label = st.selectbox("Workstation bar interval", ["5m", "15m", "30m", "1h", "4h", "1d"], index=3, help="4h is built from 1h bars when needed. Drag the chart to pan left/right and use wheel scroll to zoom.")
        live_history = get_chart_history(selected_symbol, chart_period, chart_interval_label, feed_provider)
        ...
        metric_html = [
            render_stat_card("Trend Strength", short_term["trend_strength"], "Overall outlook vs trend"),
            render_stat_card("Entry Quality", short_term["entry_quality"], "How clean today's location is"),
            render_stat_card("Buy Score", f"{short_term['buy_day_score']:.1f}", "Secondary ranking signal"),
            render_stat_card("Setup Quality", f"{safe_float(selected_row.get('setup_quality'), 0):.1f}", "Blend of trend, entry, risk, and pullback quality"),
        ]
        ...
        st.markdown(
            ...
            f'<span class="small-note">2-day run: {short_term["two_day_run_pct"]:.2f}% | Today: {short_term["today_change_pct"]:.2f}% | 5-day: {short_term["five_day_pct"]:.2f}%<br>{short_term["summary"]}</span></div>',
            unsafe_allow_html=True,
        )
        ...
        with left:
            if not live_history.empty:
                ...
                st.plotly_chart(chart, use_container_width=True, config={"scrollZoom": True, "displaylogo": False, "responsive": True, "doubleClick": "reset", "showAxisDragHandles": True})
                st.caption(f"Interactive chart enabled • Window {chart_period} • Bars {chart_interval_label} • Drag to pan, scroll to zoom")
            else:
                st.caption("Live chart unavailable for this symbol and feed.")

        with right:
            render_section_header("Comparison", "section-green")
            if compare_symbol:
                st.plotly_chart(build_comparison_chart(selected_symbol, compare_symbol, chart_period, feed_provider), use_container_width=True, config={"scrollZoom": True, "displaylogo": False, "responsive": True, "doubleClick": "reset", "showAxisDragHandles": True})
            ...
            render_section_header("12-month scenario estimates", "section-gold")
            a1, a2, a3 = st.columns(3)
            annual_cards = [
                render_stat_card("Bull target", f"${annual_est['bull_target']:,.2f}", f"{annual_est['bull_return_pct']:.1f}%"),
                render_stat_card("Base target", f"${annual_est['base_target']:,.2f}", f"{annual_est['base_return_pct']:.1f}%"),
                render_stat_card("Bear target", f"${annual_est['bear_target']:,.2f}", f"{annual_est['bear_return_pct']:.1f}%"),
            ]
            for col, html in zip([a1, a2, a3], annual_cards):
                with col:
                    st.markdown(html, unsafe_allow_html=True)
            st.caption(annual_est["summary"])

            render_section_header("Custom formula read", "section-cyan")
            f1, f2, f3, f4 = st.columns(4)
            formula_cards = [
                render_stat_card("Pullback Quality", f"{safe_float(selected_row.get('pullback_quality'), 0):.1f}", "How usable the dip looks"),
                render_stat_card("Risk-Adjusted Entry", f"{safe_float(selected_row.get('risk_adjusted_entry'), 0):.1f}", "Reward vs support / ATR risk"),
                render_stat_card("Extension Score", f"{safe_float(selected_row.get('extension_score'), 0):.1f}", "Higher is less overheated"),
                render_stat_card("Trend vs Entry Gap", f"{safe_float(selected_row.get('trend_entry_divergence'), 0):.1f}", "Good asset, bad entry detector"),
            ]
            for col, html in zip([f1, f2, f3, f4], formula_cards):
                with col:
                    st.markdown(html, unsafe_allow_html=True)

            if include_news:
                with st.expander("Selected ticker headlines", expanded=False):
            ...
            if not live_history.empty:
                ...
                render_section_header("Trade plan", "section-magenta")
                row_one = st.columns(4)
                row_two = st.columns(3)
                cards_one = [
                    render_stat_card("Entry", f"${plan['entry_price']:,.2f}"),
                    render_stat_card("Stop", f"${plan['stop_price']:,.2f}"),
                    render_stat_card("Target", f"${plan['target_price']:,.2f}"),
                    render_stat_card("R:R", f"{plan['rr_ratio']:.2f}"),
                ]
                cards_two = [
                    render_stat_card("Suggested units", f"{plan['units']:,.4f}"),
                    render_stat_card("Position value", f"${plan['position_value']:,.2f}"),
                    render_stat_card("Capital at risk", f"${plan['capital_at_risk']:,.2f}"),
                ]
                for col, html in zip(row_one, cards_one):
                    with col:
                        st.markdown(html, unsafe_allow_html=True)
                for col, html in zip(row_two, cards_two):
                    with col:
                        st.markdown(html, unsafe_allow_html=True)

        st.subheader("Short-term swing read")
        trend_strength = str(selected_row.get("trend_strength", derive_trend_strength_label(safe_float(selected_row.get("mtf_score"), 50.0))))
        entry_quality = str(selected_row.get("entry_quality_label", "Fair Entry"))
        s1, s2, s3 = st.columns(3)
        with s1:
            st.markdown(render_stat_card("5-day momentum", f"{short_term['five_day_pct']:.2f}%", "Recent pace"), unsafe_allow_html=True)
        with s2:
            st.markdown(render_stat_card("Trend Strength", trend_strength, str(selected_row.get("mtf_label", "-"))), unsafe_allow_html=True)
        with s3:
            st.markdown(render_stat_card("Entry Quality", entry_quality, f"Buy Score {short_term['buy_day_score']:.1f}"), unsafe_allow_html=True)

        st.caption(
            "Trend Strength tells you whether the asset backdrop is healthy. Entry Quality tells you whether the current price looks usable. "
            "Buy Score is secondary and mainly helps rank dip setups instead of replacing trend and entry context."
        )
...
with tab3:
    render_section_header("Private journal and alerts", "section-magenta")
    st.caption("This area is private to the signed-in account and stores saved alerts plus trade notes. Other users should only see their own saved items.")
    st.markdown('<div class="privacy-card"><strong>Private saved tools</strong><br><span class="small-note">Alerts and journal entries are tied to the signed-in account. Other users do not see your entries.</span></div>', unsafe_allow_html=True)
    if not get_current_user_id():
        ...
        if not saved_alerts.empty:
            st.dataframe(saved_alerts[[c for c in ["symbol","alert_type","target_value","note","is_active","created_at"] if c in saved_alerts.columns]], use_container_width=True, hide_index=True)
        else:
        ...
        journal_thesis = st.text_area("Thesis / notes", height=120, help="This is your private note for why you liked or avoided the setup.")
        ...
        if not journal_df.empty:
            st.dataframe(journal_df[[c for c in ["symbol","side","entry_price","stop_price","target_price","status","created_at"] if c in journal_df.columns]], use_container_width=True, hide_index=True)
        else:
            st.caption("No journal entries yet.")

with tab4:
    render_section_header("Settings", "section-cyan")
    ...
    st.markdown('<div class="accent-card-soft"><strong>Speed-first data mode</strong><br><span class="small-note">NeuroTrade v11 keeps scans and charts on Yahoo Finance by default so the workstation stays responsive. Optional premium feeds can be reintroduced later if you want more real-time precision.</span></div>', unsafe_allow_html=True)
    st.caption(f"Project folder: {BASE_DIR}")

csv_data = filtered.to_csv(index=False).encode("utf-8")
st.download_button("Download results as CSV", data=csv_data, file_name="neurotrade_scan_results.csv", mime="text/csv", use_container_width=True)
