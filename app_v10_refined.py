...
.main-title { color: #fffdf6; font-size: 2.15rem; font-weight: 900; margin-bottom: 0.15rem; text-shadow: 0 0 18px rgba(204,255,95,0.30); }
...
.crypto-summary-card {
    background: linear-gradient(180deg, rgba(84, 62, 9, 0.96) 0%, rgba(49, 34, 8, 0.99) 100%);
    border-left: 6px solid #ffef6e;
    color: #fff7a8;
    box-shadow: 0 0 22px rgba(255, 234, 96, 0.22);
    border-radius: 18px; padding: 0.95rem 1rem;
}
.stock-summary-card {
    background: linear-gradient(180deg, rgba(25, 42, 45, 0.96) 0%, rgba(19, 31, 34, 0.99) 100%);
    border-left: 6px solid #73ffd8;
    color: #ebfff9;
    border-radius: 18px; padding: 0.95rem 1rem;
}
...
div[data-testid="stMetric"] {
    background: linear-gradient(180deg, rgba(16, 31, 19, 0.95) 0%, rgba(15, 24, 17, 0.98) 100%);
    border-radius: 18px;
    padding: 0.85rem;
    border: 1px solid rgba(164, 255, 74, 0.28);
    box-shadow: 0 0 18px rgba(122, 255, 46, 0.12);
}
div[data-testid="stMetricLabel"] * {
    color: #b9ff39 !important;
    font-weight: 800 !important;
    text-shadow: 0 0 10px rgba(165, 255, 88, 0.35);
}
div[data-testid="stMetricValue"] * {
    color: #e9ffd0 !important;
    font-weight: 900 !important;
}
div[data-testid="stMetricDelta"] * { color: #d8ff98 !important; }
...
def notify_breaking_news_from_scan(result: pd.DataFrame, include_news: bool, fresh_scan: bool) -> None:
    if not include_news or not fresh_scan or result.empty:
        return
    seen = set(st.session_state.get("seen_headlines", []))
    new_items = []
    for _, row in result.head(12).iterrows():
        headlines = str(row.get("top_headlines", "") or "").split(" || ")
        first = headlines[0].strip() if headlines else ""
        if not first or first in seen:
            continue
        if str(row.get("news_sentiment_label", "Neutral")) == "Neutral":
            continue
        new_items.append((str(row.get("symbol", "")), first))
    for symbol, headline in new_items[:3]:
        try:
            st.toast(f"Breaking watchlist news — {symbol}: {headline[:110]}")
        except Exception:
            st.markdown(f'<div class="news-banner"><strong>Breaking watchlist news — {symbol}</strong><br>{headline}</div>', unsafe_allow_html=True)
        seen.add(headline)
    st.session_state["seen_headlines"] = list(seen)[-50:]
...
with tab1:
    ...
    cols = [c for c in ["symbol","asset_class","price","decision","trend_strength","entry_quality_label","best_buy_today_rank","buy_day_score","buy_day_label","entry_score","strength_score","preferred_buy_price","distance_from_buy_pct","today_change_pct","two_day_run_pct","mtf_label","news_sentiment_label"] if c in top.columns]
...
with tab2:
    ...
    if not symbols:
        ...
        metric_html = [
            ...
            render_stat_card("Short-term read", short_term["buy_day_label"], short_term.get("buy_day_hint", "")),
        ]
        ...
        with left:
            if not live_history.empty:
                ...
                st.caption(f"Interactive chart enabled • Window {chart_period} • Bars {chart_interval_label}")
            else:
