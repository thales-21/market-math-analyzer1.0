...
st.markdown(
    ...
    div[data-testid="stMetric"] {{
        background: linear-gradient(180deg, rgba(26, 54, 74, 0.98) 0%, rgba(18, 26, 42, 0.99) 100%);
        border: 1px solid {ACCENT_BORDER};
        border-radius: 18px; padding: 0.95rem; box-shadow: 0 12px 30px rgba(0,0,0,0.30);
    }}
    div[data-testid="stMetricLabel"] {{ color: #aaff9c !important; font-weight: 900; opacity: 1 !important; text-shadow: 0 0 14px rgba(170,255,156,0.60); letter-spacing: 0.01em; }}
    div[data-testid="stMetricValue"] {{ color: #f5fff2 !important; font-size: 1.48rem !important; font-weight: 900; text-shadow: 0 0 10px rgba(170,255,156,0.18); }}
    .header-box {{
    ...
    .stat-card.crypto {{
        background: linear-gradient(180deg, rgba(56, 44, 6, 0.96) 0%, rgba(32, 25, 14, 0.98) 100%);
        border: 1px solid rgba(240, 216, 106, 0.44); color: #fff7d5;
        box-shadow: 0 10px 24px rgba(0,0,0,0.22), 0 0 18px rgba(240,216,106,0.10);
    }}
    .stat-card.stock {{
        background: linear-gradient(180deg, rgba(18, 39, 57, 0.96) 0%, rgba(20, 24, 38, 0.98) 100%);
        border: 1px solid {ACCENT_BORDER}; color: #effcff;
    ...
    .stat-card.crypto .stat-title {{ color: #ffe88a; text-shadow: 0 0 10px rgba(240,216,106,0.35); }}
    .stat-card.stock .stat-title {{ color: {ACCENT}; text-shadow: 0 0 10px rgba(120,217,255,0.22); }}
    ...
    .pill-crypto {{ background: rgba(240,216,106,0.16); border:1px solid rgba(240,216,106,0.42); color:#ffe88a; }}
    .pill-stock {{ background: rgba(120,217,255,0.16); border:1px solid {ACCENT_BORDER}; color:{ACCENT}; }}
...
def styled_scan_table(df: pd.DataFrame):
    def asset_style(v):
        return "background-color: rgba(240,216,106,0.16); color: #6b5600; font-weight: 800;" if str(v) == "Crypto" else "background-color: rgba(120,217,255,0.16); color: #0a4c66; font-weight: 800;"

    def entry_style(v):
        m = {
            "Strong Buy": "background-color: rgba(170,255,156,0.18); color:#1e5f14; font-weight:800;",
            "Weak Buy": "background-color: rgba(125,235,255,0.18); color:#0a4c66; font-weight:800;",
            "Hold / Wait": "background-color: rgba(255,228,107,0.20); color:#705600; font-weight:800;",
            "Sell / Avoid": "background-color: rgba(255,146,165,0.18); color:#7a1f32; font-weight:800;",
        }
        return m.get(str(v), "")

    def transition_style(v):
        m = {
            "Improving": "background-color: rgba(170,255,156,0.16); color:#1e5f14; font-weight:800;",
            "Weakening": "background-color: rgba(255,146,165,0.16); color:#7a1f32; font-weight:800;",
            "Unchanged": "background-color: rgba(120,217,255,0.14); color:#0a4c66; font-weight:700;",
            "New": "background-color: rgba(255,228,107,0.16); color:#705600; font-weight:700;",
        }
        return m.get(str(v), "")

    styler = df.style.map(asset_style, subset=["asset_class"])
    styler = styler.map(entry_style, subset=["entry_quality"])
    if "signal_transition" in df.columns:
...
with dashboard_tab:
    ...
    for col, (_, row) in zip(cols, top.iterrows()):
        ...
        card_class = "gold-box" if row["asset_class"] == "Crypto" else "soft-box"
        col.markdown(
            f'<div class="{card_class}"><strong>{row["symbol"]}</strong><br>'
            f'<span class="pill {asset_pill}">{row["asset_class"]}</span>'
...
with workstation_tab:
    ...
    selected_theme = asset_theme(selected_row.get("asset_class", "Stock / ETF"))
    selected_header_class = "gold-box" if selected_theme == "crypto" else "soft-box"
    ...
    if chart_history.empty:
        ...
        with work_right:
            st.markdown(f'<div class="{selected_header_class}"><strong>{selected_symbol}</strong> · <span class="small-note">{selected_row.get("asset_class", "-")}</span><br><span class="small-note">Theme follows the selected asset class for faster visual reading.</span></div>', unsafe_allow_html=True)
            short_term = compute_short_term_context(daily_history if not daily_history.empty else chart_history, selected_row)
            st.markdown(stat_card("Trend Strength", str(short_term.get("trend_strength", "-")), short_term.get("read_text", ""), selected_theme), unsafe_allow_html=True)
            st.markdown(stat_card("Entry Quality", str(short_term.get("entry_quality", "-")), f"Preferred buy {format_num(selected_row['preferred_buy_price'], prefix='$')} | Distance {safe_float(selected_row['distance_from_buy_pct']):+.1f}%", selected_theme), unsafe_allow_html=True)
            st.markdown(stat_card("Buy Score", f"{safe_float(short_term.get('buy_score'), 0):.1f}", "Secondary ranking signal, not a chase signal.", selected_theme), unsafe_allow_html=True)
            st.markdown(stat_card("Setup Name", str(selected_row.get("setup_name", "Balanced Setup")), str(selected_row.get("why_signal", "")), selected_theme), unsafe_allow_html=True)
            st.markdown(stat_card("Signal Memory", str(selected_row.get("signal_transition", "Unchanged")), f"Preferred buy change {safe_float(selected_row.get('preferred_buy_change_pct'), 0):+.2f}%", selected_theme), unsafe_allow_html=True)

            scenarios = compute_annual_scenarios(daily_history if not daily_history.empty else chart_history, selected_row)
            st.markdown(f'<div class="section-header">12-month scenario estimates · {selected_row.get("asset_class", "")} theme</div>', unsafe_allow_html=True)
            s1, s2, s3 = st.columns(3)
            s1.markdown(stat_card("Bear", scenarios["bear"], "Lower path", selected_theme), unsafe_allow_html=True)
            s2.markdown(stat_card("Base", scenarios["base"], "Expected path", selected_theme), unsafe_allow_html=True)
            s3.markdown(stat_card("Bull", scenarios["bull"], "High path", selected_theme), unsafe_allow_html=True)

            plan = estimate_trade_plan(selected_row)
            st.markdown(f'<div class="section-header">Trade plan · {selected_row.get("asset_class", "")} theme</div>', unsafe_allow_html=True)
            p0, p1, p2 = st.columns(3)
            p0.markdown(stat_card("Current Price", f"${safe_float(selected_row['price']):,.2f}", f"Preferred buy ${plan['entry']:,.2f}", selected_theme), unsafe_allow_html=True)
            p1.markdown(stat_card("Entry", f"${plan['entry']:,.2f}", f"Target ${plan['target']:,.2f}", selected_theme), unsafe_allow_html=True)
            p2.markdown(stat_card("Stop", f"${plan['stop']:,.2f}", f"R:R {plan['rr']:.2f}", selected_theme), unsafe_allow_html=True)

            st.markdown(f'<div class="section-header">Formula layer · {selected_row.get("asset_class", "")} theme</div>', unsafe_allow_html=True)
            f1, f2 = st.columns(2)
            f1.markdown(stat_card("Setup Quality", f"{safe_float(selected_row['setup_quality']):.1f}", "Trend + entry blend", selected_theme), unsafe_allow_html=True)
            f2.markdown(stat_card("Pullback Quality", f"{safe_float(selected_row['pullback_quality']):.1f}", "2-day run + current pullback", selected_theme), unsafe_allow_html=True)
            f3, f4 = st.columns(2)
            f3.markdown(stat_card("Risk-Adjusted Entry", f"{safe_float(selected_row['risk_adjusted_entry']):.1f}", "Reward vs stop risk", selected_theme), unsafe_allow_html=True)
            f4.markdown(stat_card("Extension Score", f"{safe_float(selected_row['extension_score']):.1f}", "Higher = more extended", selected_theme), unsafe_allow_html=True)
            st.markdown(stat_card("Trend vs Entry Gap", f"{safe_float(selected_row['trend_entry_gap']):+.1f}", "Positive means trend is better than the entry right now.", selected_theme), unsafe_allow_html=True)
            st.markdown(stat_card("Why This Signal", str(selected_row.get("entry_quality", "-")), str(selected_row.get("why_signal", "")), selected_theme), unsafe_allow_html=True)
