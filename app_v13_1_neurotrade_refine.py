...
def infer_preferred_buy(symbol: str, history: pd.DataFrame, current_price: float, support: float) -> float:
    ...
    if history.empty:
        if manual_preferred > 0:
            return round(min(manual_preferred, current_price * 0.997), 2)
    ...
    support_w = safe_float(cfg.get("support_weight"), 0.30)
    ema21_w = safe_float(cfg.get("ema21_weight"), 0.20)
    low20_w = safe_float(cfg.get("low20_weight"), 0.25)
    manual_w = safe_float(cfg.get("manual_weight"), 0.25)

    adaptive = (support * support_w) + (ema21 * ema21_w) + (low20 * low20_w) + (low60 * max(0.0, 1 - support_w - ema21_w - low20_w - manual_w))
    if manual_preferred > 0:
        preferred = (adaptive * (1 - manual_w)) + (manual_preferred * manual_w)
        if adaptive < manual_preferred * 0.97:
            preferred = (manual_preferred * 0.68) + (adaptive * 0.32)
        elif adaptive > manual_preferred * 1.04:
            preferred = (manual_preferred * 0.40) + (adaptive * 0.60)
    else:
        preferred = adaptive

    preferred *= 1 - min(max(atr_pct * 0.14, 0.0), 0.012)
    floor = max(current_price * 0.89, support * 0.992)
    ceiling = min(current_price * 0.996, ema21 * 1.002)
    if manual_preferred > 0:
        ceiling = min(ceiling, manual_preferred * 1.012)
        floor = max(floor, manual_preferred * 0.965)
