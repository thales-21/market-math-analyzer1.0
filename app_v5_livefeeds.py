...
def analyze_pullback_setup(symbol: str, current_price: float, rsi: float, macd: float, macd_signal: float, preferred_buy: float, support: float, resistance: float, mtf_score: float, news_score: float, atr_pct: float | None = None, volume_ratio: float | None = None) -> dict:
    ...
    score = 50.0 + (12 if macd > macd_signal else -12)
    score += 18 if cfg["rsi_buy_min"] <= rsi <= cfg["rsi_buy_max"] else -18 if rsi > cfg["rsi_hot"] else 0
    score += 20 if dist <= 0 else 10 if dist <= cfg["max_chase_pct"] else -25 if dist > cfg["hard_overextended_pct"] else 0
    score += 10 if support*0.995 <= current_price <= support*1.03 else 0
    score += -8 if resistance and (resistance-current_price)/current_price < 0.05 else 0
    if atr_pct is not None and not pd.isna(atr_pct): score += 8 if atr_pct <= cfg["volatility_tolerance"] else -8
    if volume_ratio is not None and not pd.isna(volume_ratio): score += 8 if volume_ratio >= 1.1 else -5 if volume_ratio < 0.85 else 0
    score += (mtf_score-50)*0.2 + news_score*8
    final = round(clamp(score,0,100),1)
    decision = "BUY" if final >= 72 else "HOLD / WAIT" if final >= 45 else "AVOID"
    return {"decision":decision,"signal":decision,"entry_score":final,"score":final,"entry_quality":label_strength(final),"confidence":"High" if final >= 75 else "Medium" if final >= 55 else "Low","preferred_buy_price":preferred_buy,"distance_from_buy_pct":round(dist*100,2),"wait_price":round(preferred_buy if current_price > preferred_buy else current_price,2),"notes":f"RSI {rsi:.1f} | MACD {'constructive' if macd > macd_signal else 'weak'} | Support {support:.2f} | Resistance {resistance:.2f}"}
...
def enrich_results_with_pullback_system(df: pd.DataFrame, period: str, include_news: bool = True) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        row["price"] = round(current_price,2); row["decision"] = pullback["decision"]; row["signal"] = pullback["signal"]; row["entry_score"] = pullback["entry_score"]; row["score"] = pullback["score"]; row["confidence"] = pullback["confidence"]; row["entry_quality"] = pullback["entry_quality"]; row["preferred_buy_price"] = pullback["preferred_buy_price"]; row["distance_from_buy_pct"] = pullback["distance_from_buy_pct"]; row["wait_price"] = pullback["wait_price"]; row["notes"] = pullback["notes"]; row["support"] = levels["support"]; row["resistance"] = levels["resistance"]; row["mtf_score"] = mtf["mtf_score"]; row["mtf_label"] = mtf["mtf_label"]; row["mtf_hourly"] = mtf["mtf_hourly"]; row["mtf_daily"] = mtf["mtf_daily"]; row["mtf_weekly"] = mtf["mtf_weekly"]; row["news_sentiment_score"] = news["news_sentiment_score"]; row["news_sentiment_label"] = news["news_sentiment_label"]; row["news_headline_count"] = news["news_headline_count"]; row["strength_score"] = round((safe_float(row["entry_score"],50)*0.6)+(safe_float(row["mtf_score"],50)*0.25)+((50+safe_float(row["news_sentiment_score"],0)*60)*0.15),1)
        rows.append(row)
...
def build_share_text(result: pd.DataFrame, crypto_title: str, stock_title: str) -> str:
    ...
    if not result.empty:
        top = result.sort_values(["entry_score"], ascending=False).head(5); lines.append("Top setups:")
...
with st.spinner("Scanning pullback setups with structure, timeframes, and headlines..."):
    result = get_analysis(period=period, interval="1d"); result = enrich_results_with_pullback_system(result, period=period, include_news=include_news)
if result.empty:
...
with c4: st.metric("Feed", feed_provider)
...
with tab1:
    ...
    top = filtered.sort_values(["entry_score"], ascending=False).head(10)
    cols = [c for c in ["symbol","asset_class","price","decision","entry_score","strength_score","preferred_buy_price","distance_from_buy_pct","support","resistance","mtf_label","news_sentiment_label"] if c in top.columns]
    ...
    if not symbols:
        ...
        with left:
            if not live_history.empty:
                chart = build_advanced_chart(live_history.tail(350), selected_symbol, safe_float(selected_row.get("preferred_buy_price"),0.0), safe_float(selected_row.get("support"),0.0), safe_float(selected_row.get("resistance"),0.0), overlays={"SMA20":show_sma20,"SMA50":show_sma50,"EMA9":show_ema9,"EMA21":show_ema21,"EMA50":show_ema50}, show_rsi=show_rsi, show_macd=show_macd)
        ...
        with right:
            ...
            if compare_symbol:
                ...
                compare_table = filtered[filtered["symbol"].isin([selected_symbol, compare_symbol])][[c for c in ["symbol","price","decision","entry_score","strength_score","mtf_score","news_sentiment_label","preferred_buy_price"] if c in filtered.columns]].copy()
                st.dataframe(compare_table, use_container_width=True, hide_index=True)
            if include_news:
                news = get_news_sentiment(selected_symbol)
                st.subheader("Headline sentiment"); st.caption(f"{news['news_sentiment_label']} | score {news['news_sentiment_score']:.3f} | headlines {news['news_headline_count']}")
                for headline in news.get("top_headlines", [])[:5]: st.write(f"- {headline}")
            if not live_history.empty:
                plan = estimate_trade_plan(safe_float(selected_row.get("price"),0.0), safe_float(selected_row.get("preferred_buy_price"),0.0), safe_float(selected_row.get("support"),0.0), safe_float(selected_row.get("resistance"),0.0), safe_float(live_history["ATR_PCT"].tail(1).mean(),0.03), account_size, risk_pct, max_exposure_pct)
                st.subheader("Trade plan")
                st.metric("Entry", f"${plan['entry_price']:,.2f}"); st.metric("Stop", f"${plan['stop_price']:,.2f}"); st.metric("Target", f"${plan['target_price']:,.2f}"); st.metric("R:R", f"{plan['rr_ratio']:.2f}")
                st.metric("Suggested units", f"{plan['units']:,.4f}"); st.metric("Position value", f"${plan['position_value']:,.2f}"); st.metric("Capital at risk", f"${plan['capital_at_risk']:,.2f}")
...
with tab4:
    ...
    with cw2:
        if st.button("Save watchlist"):
            lines = editable_watchlist.splitlines(); lines.extend(chosen_presets); ok, msg = save_user_watchlist(lines); st.success(msg) if ok else st.info(msg)
    ...
    if st.button("Save formulas"):
        ok, msg = save_user_formulas(formulas_text); st.success(msg) if ok else st.info(msg)
