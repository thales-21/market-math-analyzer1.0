...
def render_stat_card(label: str, value: str, sub: str = "") -> str:
    return f'<div class="stat-card"><div class="stat-label">{label}</div><div class="stat-value">{value}</div><div class="stat-sub">{sub}</div></div>'
...
def get_chart_history(symbol: str, chart_window: str, bar_interval: str, feed: str) -> pd.DataFrame:
    if bar_interval == "4h":
        base_window = "3mo" if chart_window in {"5d", "1mo"} else chart_window
        history = get_symbol_history(symbol, base_window, feed)
        return resample_ohlcv(history, "4h")
    return get_symbol_history(symbol, chart_window, feed)
...
with tab2:
    ...
    if not symbols:
        ...
        chart_interval_label = st.selectbox("Workstation bar interval", ["15m", "1h", "4h", "1d"], index=2, help="4h is built by resampling lower-timeframe candles when needed.")
        ...
        metric_html = [
            render_stat_card("2-day run before today", f"{short_term['two_day_run_pct']:.2f}%", "Recent push into today"),
            render_stat_card("Today / latest daily move", f"{short_term['today_change_pct']:.2f}%", short_term["pullback_type"]),
        ...
        with left:
            if not live_history.empty:
                ...
                st.plotly_chart(chart, use_container_width=True, config={"scrollZoom": True, "displaylogo": False, "responsive": True})
        ...
        with right:
            ...
            if compare_symbol:
                st.plotly_chart(build_comparison_chart(selected_symbol, compare_symbol, chart_period, feed_provider), use_container_width=True, config={"scrollZoom": True, "displaylogo": False, "responsive": True})
