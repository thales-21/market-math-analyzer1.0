...
st.set_page_config(page_title="NeuroTrade v16.1", layout="wide")
...
def auth_sign_in(email: str, password: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        if "Invalid login credentials" in message:
            message += " If you recently signed up, check whether your email needs confirmation first. You can also use the reset option below."
...
def auth_sign_up(email: str, password: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        return True, "Account created. Check your email if confirmation is required before login."
    except Exception as exc:
...
def auth_reset_password(email: str) -> Tuple[bool, str]:
    ...
    redirect_to = get_secret_value("PASSWORD_RESET_REDIRECT_URL", "").strip()
    try:
        ...
        if hasattr(auth_api, "reset_password_for_email"):
            ...
            auth_api.reset_password_for_email(**kwargs)
        elif hasattr(auth_api, "reset_password_email"):
            if redirect_to:
                auth_api.reset_password_email(email, {"redirect_to": redirect_to})
            else:
                auth_api.reset_password_email(email)
        else:
            return False, "Password reset is not available with the installed Supabase client."
...
st.markdown('<div class="main-title">NeuroTrade v16.1</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-title">NeuroTrade thinks like market neurons: current-week entry logic, sunset glass styling, a dark neon scan table, and a faster futuristic workstation.</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if get_supabase_client() is None:
        ...
        auth_mode = st.radio("Account action", ["Login", "Sign up", "Reset password"], horizontal=True)
        if auth_mode in {"Login", "Sign up"}:
            ...
            if submitted:
                ok, msg = auth_sign_in(email.strip(), password) if auth_mode == "Login" else auth_sign_up(email.strip(), password)
                if ok:
                    st.success(msg)
                    if auth_mode == "Login":
                        st.rerun()
            ...
            if reset_submitted:
                ok, msg = auth_reset_password(reset_email)
                if ok:
                    st.success(msg)
                else:
                    st.error(msg)
            st.caption("This sends a password reset email through Supabase.")
