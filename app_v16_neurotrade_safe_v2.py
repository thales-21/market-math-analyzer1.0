        ...
        if submitted:
            ...
            if mode == "Login":
                ...
                if ok:
                    ...
                    st.error(msg)
                    st.caption("If you forgot your password, switch to Reset password.")
            elif mode == "Sign up":
...
with workstation_tab:
    ...
    if chart_history.empty:
        ...
        with work_right:
            ...
            p1.markdown(stat_card("Current Price", format_num(selected_row["price"], prefix="$"), "Live selected ticker price", theme), unsafe_allow_html=True)
            p2.markdown(stat_card("Entry", format_num(plan["entry"], prefix="$"), "Preferred entry", theme), unsafe_allow_html=True)
            p3.markdown(stat_card("Stop", format_num(plan["stop"], prefix="$"), "Risk floor", theme), unsafe_allow_html=True)
            p4, p5 = st.columns(2)
            p4.markdown(stat_card("Target", format_num(plan["target"], prefix="$"), "Reward zone", theme), unsafe_allow_html=True)
            p5.markdown(stat_card("R/R", f"{plan['rr']:.2f}", "Target ÷ risk", theme), unsafe_allow_html=True)
